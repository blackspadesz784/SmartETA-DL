"""
Run this LOCALLY (not on Render) to train the model:

    pip install -r requirements-train.txt
    python train_local.py

Output: backend/model_cache/ folder containing model.keras, scaler.pkl,
encoders.pkl, metadata.pkl. Commit this folder to your repo — Render will
just load it, no training happens there.
"""
...
FEATURE_ORDER = [
    ...
    "Type_of_vehicle", "multiple_deliveries", "Festival", "City", "Day", "Month",
...
def main():
    if not os.path.exists(CSV_PATH):
        raise FileNotFoundError(f"train.csv not found at {CSV_PATH}. Place it next to train_local.py.")

    raw = pd.read_csv(CSV_PATH)
    df = clean_dataframe(raw)

    eda_numeric_cols = [c for c in df.columns if pd.api.types.is_numeric_dtype(df[c])]
    eda_graphs = make_eda_graphs(
        df[eda_numeric_cols], df["City"], df["Delivery_person_Age"], df["Time_taken(min)"]
    )
    ...
    plt.xlabel("Epoch"); plt.ylabel("Loss (MSE)"); plt.title("Training History"); plt.legend()
    ...
    plt.xlabel("Test sample #"); plt.ylabel("Delivery Time (min)")
    plt.title("Actual vs Predicted (first 20 test samples)")
    plt.xticks(x); plt.legend()
    ...
    categorical_options = {col: sorted(encoders[col].classes_.tolist()) for col in CATEGORICAL_COLS}
    ...
    with open(METADATA_PATH, "wb") as f:
        pickle.dump({
            "metrics": metrics, "eda_graphs": eda_graphs,
            "model_graphs": model_graphs, "categorical_options": categorical_options,
        }, f)

    print("Training complete. Metrics:", metrics)
    print(f"Artefacts saved to: {CACHE_DIR}")
    print("Ab is model_cache/ folder ko git me commit karke Render pe push karo.")
